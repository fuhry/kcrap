...
void kcrap_free(struct kcrap_context *context)
{
	...
	if (context->servers)
		profile_free_list(context->servers);
	if (context->ccache)
		krb5_cc_close(context->krb5_context, context->ccache);
	if (context->sprinc)
		krb5_free_principal(context->krb5_context, context->sprinc);
	if (context->keytab)
		krb5_kt_close(context->krb5_context, context->keytab);
	if (context->profile)
		profile_release(context->profile);
	if (context->krb5_context)
		krb5_free_context(context->krb5_context);
	free(context);
	return;
}
...
#define IS_FINAL(RV) \
	((RV == 0) || (RV == EINVAL) || (RV == KRB5KDC_ERR_S_PRINCIPAL_UNKNOWN) || (RV == KRB5_KDB_NOENTRY) || (RV == KRB5KDC_ERR_KEY_EXP) || (RV == KRB5KDC_ERR_NAME_EXP) || (RV == KRB5KDC_ERR_PRINCIPAL_NOT_UNIQUE))
...
int kcrap_try(struct kcrap_context *context, struct kcrap_auth_req_data *req, int *auth_status)
{
	...
	*auth_status = 0;
	...
	for (trycount = 0; trycount < 10 && !IS_FINAL(ok);)
	{
		for (i = 0; context->servers[i] && !IS_FINAL(ok); i++, trycount++)
		{
			...
			*portstr = '\0';
			...
			{
				if (kcrap_send(context, sock, &to, KCRAP_PKT_AUTH_REQ, &data, &keyblock) <= 0)
					continue;
				sent++;
	...
	if (!sent)
	{
		close(sock);
		if (keyblock)
			krb5_free_keyblock(context->krb5_context, keyblock);
		if (_errmsg[0] == '\0')
